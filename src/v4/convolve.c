  ...
  _KLT_FloatImage imgout)
{
  ...
  /* Pre-copy kernel data to device */
  float kernel_data[MAX_KERNEL_WIDTH];
  int kernel_width = kernel.width;
  for (int idx = 0; idx < kernel_width; idx++) {
    kernel_data[idx] = kernel.data[idx];
  }

  #pragma acc enter data copyin(imgin->data[0:ncols*nrows], kernel_data[0:kernel_width]) \
                       create(imgout->data[0:ncols*nrows])

  /* For each row, do ... */
  #pragma acc parallel loop gang present(imgin->data[0:ncols*nrows], \
                                        kernel_data[0:kernel_width], \
                                        imgout->data[0:ncols*nrows]) \
                          private(ptrrow, ptrout, ppp, sum, i, j, k)
  for (j = 0 ; j < nrows ; j++)  {
  ...
  #pragma acc exit data delete(imgin->data[0:ncols*nrows], kernel_data[0:kernel_width]) \
  ...
  _KLT_FloatImage imgout)
{
  ...
  #pragma acc parallel loop gang present(imgin->data[0:ncols*nrows], \
                                        kernel_data[0:kernel_width], \
                                        imgout->data[0:ncols*nrows]) \
                          private(ptrcol, ptrout, ppp, sum, i, j, k)
  for (i = 0 ; i < ncols ; i++)  {
