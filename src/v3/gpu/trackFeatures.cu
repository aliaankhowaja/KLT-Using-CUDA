					  ...
					  KLT_FeatureList featurelist)
{
	...
	if (tc->sequentialMode && tc->pyramid_last != NULL)  {
		...
		_KLTComputeSmoothedImage(tmpimg, _KLTComputeSmoothSigma(tc), floatimg1, tc->gpuImg);
		pyramid1 = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
		_KLTComputePyramid(floatimg1, pyramid1, tc->pyramid_sigma_fact, tc->gpuImg);
		...
		for (i = 0 ; i < tc->nPyramidLevels ; i++)
			...
			pyramid1_grady->img[i], tc->gpuImg);
	}

	/* Do the same thing with second image */
	floatimg2 = _KLTCreateFloatImage(ncols, nrows);
	_KLTToFloatImage(img2, ncols, nrows, tmpimg);
	_KLTComputeSmoothedImage(tmpimg, _KLTComputeSmoothSigma(tc), floatimg2, tc->gpuImg);
	pyramid2 = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
	_KLTComputePyramid(floatimg2, pyramid2, tc->pyramid_sigma_fact, tc->gpuImg);
	...
	for (i = 0 ; i < tc->nPyramidLevels ; i++)
		...
		pyramid2_grady->img[i], tc->gpuImg);
